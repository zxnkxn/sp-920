...
/* Modified from hpm6750evk2 board.c for SP-920 */
...
// Modified for SP-920
// Initialize an I2C peripheral instance in master mode.
// -----------------------------------------------------------------------------
// ptr: pointer to MCU I2C peripheral registers (e.g., HPM_I2C0, HPM_I2C1, ...).
//
// What this function does:
// 1) Enables/configures the clock for the given I2C peripheral (board-specific).
// 2) Configures the GPIO pins for I2C signals (SCL/SDA).
// 3) Sets I2C mode (standard I2C) and 7-bit addressing.
// 4) Calls the SDK driver to initialize the I2C controller as a master.
//
void board_init_i2c(I2C_Type *ptr)
{
    i2c_config_t config;   // I2C configuration structure used by the SDK driver
    hpm_stat_t stat;       // status code returned by SDK functions
    uint32_t freq;         // I2C input clock frequency (Hz) for timing configuration

    // Configure/enable the I2C peripheral clock and return the actual clock frequency.
    // The I2C driver needs this to compute proper timing values.
    freq = board_init_i2c_clock(ptr);

    // Optional: clear the bus if SDA is stuck low, etc.
    // board_i2c_bus_clear(ptr);

    // Configure the I2C pins (SCL and SDA) to the correct alternate function / mux.
    init_i2c_pins(ptr);

    // Configure I2C controller settings:
    config.i2c_mode = i2c_mode_normal;         // standard I2C operation (not SMBus, etc.)
    config.is_10bit_addressing = false;        // use 7-bit I2C addressing

    // Initialize the I2C controller as a master with the given source clock.
    stat = i2c_init_master(ptr, freq, &config);

    // If initialization fails, stop here (board bring-up style).
