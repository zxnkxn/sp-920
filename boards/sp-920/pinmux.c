...
/* Modified from hpm6750evk2 pinmux.c for SP-920 */
...
// Added for SP-920
// RS485_3: PA01 = UART1_RXD, PA02 = UART1_TXD
...
// Added for SP-920
// RS485_2: PA03 = UART3_RXD, PA00 = UART3_TXD
void init_uart3_pins(void)
{
    HPM_IOC->PAD[IOC_PAD_PA03].FUNC_CTL = IOC_PA03_FUNC_CTL_UART3_RXD;
    HPM_IOC->PAD[IOC_PAD_PA00].FUNC_CTL = IOC_PA00_FUNC_CTL_UART3_TXD;
}
...
// Added for SP-920
void init_rs485_2_dir_pin(void)
{
    ...
    HPM_IOC->PAD[IOC_PAD_PA09].PAD_CTL =
        ...
        IOC_PAD_PAD_CTL_DS_SET(6);
}

// Added for SP-920
void init_rs485_3_dir_pin(void)
{
    HPM_IOC->PAD[IOC_PAD_PA08].FUNC_CTL = IOC_PA08_FUNC_CTL_GPIO_A_08;
    HPM_IOC->PAD[IOC_PAD_PA08].PAD_CTL =
        IOC_PAD_PAD_CTL_PE_SET(1) |
        IOC_PAD_PAD_CTL_PS_SET(0) |   // pull-down
        IOC_PAD_PAD_CTL_DS_SET(6);
}
...
// Modified for SP-920: I2C0 EEPROM lines are connected to PA06 (SCL) and PA05 (SDA)
...
{
    HPM_IOC->PAD[IOC_PAD_PA06].FUNC_CTL =
        IOC_PA06_FUNC_CTL_I2C0_SCL | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;
    HPM_IOC->PAD[IOC_PAD_PA06].PAD_CTL = IOC_PAD_PAD_CTL_OD_SET(1);

    HPM_IOC->PAD[IOC_PAD_PA05].FUNC_CTL =
        IOC_PA05_FUNC_CTL_I2C0_SDA | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;
    HPM_IOC->PAD[IOC_PAD_PA05].PAD_CTL = IOC_PAD_PAD_CTL_OD_SET(1);
}
...
void init_spi0_pins(void)
{
    ...
    // SPI_ETH_SCK -> PZ03 / SPI0_SCLK
    HPM_IOC->PAD[IOC_PAD_PZ03].FUNC_CTL =
        IOC_PZ03_FUNC_CTL_SPI0_SCLK | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;
    HPM_BIOC->PAD[IOC_PAD_PZ03].FUNC_CTL = BIOC_PZ03_FUNC_CTL_SOC_PZ_03;
    HPM_IOC->PAD[IOC_PAD_PZ03].PAD_CTL = out_pad_ctl;

    // SPI_ETH_MOSI -> PZ04 / SPI0_MOSI
...
// Added for SP-920
void init_spi_eth_cs_pins(void)
{
    // Use GPIO-controlled chip selects for two KSZ8463 devices

    // SPI_ETH_CS1 -> PZ02
    HPM_IOC->PAD[IOC_PAD_PZ02].FUNC_CTL = IOC_PZ02_FUNC_CTL_GPIO_Z_02;
    HPM_BIOC->PAD[IOC_PAD_PZ02].FUNC_CTL = BIOC_PZ02_FUNC_CTL_SOC_PZ_02;
    HPM_IOC->PAD[IOC_PAD_PZ02].PAD_CTL =
        IOC_PAD_PAD_CTL_DS_SET(1) |
        IOC_PAD_PAD_CTL_PE_SET(1) |
        0x08 |
        IOC_PAD_PAD_CTL_PS_SET(1);
    ...
    HPM_IOC->PAD[IOC_PAD_PZ08].FUNC_CTL = IOC_PZ08_FUNC_CTL_GPIO_Z_08;
    HPM_BIOC->PAD[IOC_PAD_PZ08].FUNC_CTL = BIOC_PZ08_FUNC_CTL_SOC_PZ_08;
    HPM_IOC->PAD[IOC_PAD_PZ08].PAD_CTL =
        IOC_PAD_PAD_CTL_DS_SET(1) |
        IOC_PAD_PAD_CTL_PE_SET(1) |
        0x08 |
        IOC_PAD_PAD_CTL_PS_SET(1);
...
// Added for SP-920
void init_ksz_reset_pins(void)
{
    // SW1_RST -> PA14
    HPM_IOC->PAD[IOC_PAD_PA14].FUNC_CTL = IOC_PA14_FUNC_CTL_GPIO_A_14;
    HPM_IOC->PAD[IOC_PAD_PA14].PAD_CTL =
        IOC_PAD_PAD_CTL_PE_SET(1) |
        IOC_PAD_PAD_CTL_PS_SET(1) |
        IOC_PAD_PAD_CTL_DS_SET(6);
    ...
    HPM_IOC->PAD[IOC_PAD_PA17].PAD_CTL =
        IOC_PAD_PAD_CTL_PE_SET(1) |
        IOC_PAD_PAD_CTL_PS_SET(1) |
        IOC_PAD_PAD_CTL_DS_SET(6);
}
...
// Added for SP-920
void init_ksz_interrupt_pins(void)
{
    // ETH0_INT -> PA15
    HPM_IOC->PAD[IOC_PAD_PA15].FUNC_CTL = IOC_PA15_FUNC_CTL_GPIO_A_15;
    HPM_IOC->PAD[IOC_PAD_PA15].PAD_CTL =
        IOC_PAD_PAD_CTL_PE_SET(1) |
        IOC_PAD_PAD_CTL_PS_SET(1) |
        IOC_PAD_PAD_CTL_DS_SET(1);
...
{
    // REF0_50MHz output -> PB15 / SOC_REF0
    HPM_IOC->PAD[IOC_PAD_PB15].FUNC_CTL = IOC_PB15_FUNC_CTL_SOC_REF0;
    HPM_IOC->PAD[IOC_PAD_PB15].PAD_CTL = IOC_PAD_PAD_CTL_DS_SET(6);

    // Also route REFCLK internally to ENET0
    HPM_IOC->PAD[IOC_PAD_PE31].FUNC_CTL =
        IOC_PE31_FUNC_CTL_ETH0_REFCLK | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;
...
{
    // REF1_50MHz -> PE24 / SOC_REF1
    HPM_IOC->PAD[IOC_PAD_PE24].FUNC_CTL = IOC_PE24_FUNC_CTL_SOC_REF1;
    HPM_IOC->PAD[IOC_PAD_PE24].PAD_CTL = IOC_PAD_PAD_CTL_DS_SET(6);

    // Also route REFCLK internally to ENET1
    HPM_IOC->PAD[IOC_PAD_PE16].FUNC_CTL =
        IOC_PE16_FUNC_CTL_ETH1_REFCLK | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;

    // ENET1 RMII
    // ETH1_TXD1 -> D15 -> PD16
    HPM_IOC->PAD[IOC_PAD_PD16].FUNC_CTL = IOC_PD16_FUNC_CTL_ETH1_TXD_1;

    // ETH1_RXD0 -> D17 -> PD18
    HPM_IOC->PAD[IOC_PAD_PD18].FUNC_CTL = IOC_PD18_FUNC_CTL_ETH1_RXD_0;

    // ETH1_TXEN -> E14 -> PD19
    HPM_IOC->PAD[IOC_PAD_PD19].FUNC_CTL = IOC_PD19_FUNC_CTL_ETH1_TXEN;

    // ETH1_RXDV -> E16 -> PD21
    HPM_IOC->PAD[IOC_PAD_PD21].FUNC_CTL = IOC_PD21_FUNC_CTL_ETH1_RXDV;

    // ETH1_TXD0 -> F15 -> PD25
    HPM_IOC->PAD[IOC_PAD_PD25].FUNC_CTL = IOC_PD25_FUNC_CTL_ETH1_TXD_0;

    // ETH1_RXD1 -> F17 -> PD27
    HPM_IOC->PAD[IOC_PAD_PD27].FUNC_CTL = IOC_PD27_FUNC_CTL_ETH1_RXD_1;
}
