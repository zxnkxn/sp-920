...
/*
 * Source:
 * hpm_sdk/samples/lwip/common/multiple/common.c
 */
...
{
    enet_mac_config_t        enet_config = {0};
    ...
    //memcpy(&desc[idx].tx_control_config,
    //       &enet_tx_control_config,
    //       sizeof(enet_tx_control_config_t));

    enet_get_default_tx_control_config(base, &enet_tx_control_config);

    /* SP-920: checksum считаем программно, аппаратную вставку checksum отключаем */
    ...
    st = sp920_read_mac_from_eeprom(idx, mac[idx]);

    if (st != status_success) {
        printf("ENET%d EEPROM MAC read failed, status=%d\r\n", idx, st);
        return status_fail;
    }
    ...
    //printf("ENET%d MAC = %02x:%02x:%02x:%02x:%02x:%02x\r\n",
    //       idx,
    //       mac[idx][0], mac[idx][1], mac[idx][2],
    //       mac[idx][3], mac[idx][4], mac[idx][5]);

    //printf("ENET%d mac_high=0x%08lx mac_low=0x%08lx valid_max_count=%lu dma_pbl=%lu sarc=%lu itf=%d\r\n",
    //       idx,
    //       enet_config.mac_addr_high[0],
    //       enet_config.mac_addr_low[0],
    //       enet_config.valid_max_count,
    //       enet_config.dma_pbl,
    //       enet_config.sarc,
    //       itf);
    ...
    st = enet_controller_init(base, itf, &desc[idx], &enet_config, &int_config);

    if (st != status_success) {
        printf("Enet%d MAC init failed, status=%d\r\n", idx, st);
        return status_fail;
    }

    enet_set_line_speed(base, enet_line_speed_100mbps);
    enet_set_duplex_mode(base, enet_full_duplex);
...
void enet_self_adaptive_port_speed(void)
{
    ...
    static bool printed_once[BOARD_ENET_COUNT] = { false };
    ...
    for (uint8_t i = 0; i < BOARD_ENET_COUNT; i++) {
        base = board_get_enet_base(i);
        board_get_enet_phy_status(i, &status);

        if (!printed_once[i] ||
            memcmp(&last_status[i], &status, sizeof(enet_phy_status_t)) != 0) {
